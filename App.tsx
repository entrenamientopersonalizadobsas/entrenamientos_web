...
function App() {
    ...
    // Estado para los valores del check-in diario
    const [estadoAnimo, setEstadoAnimo] = useState('');
    const [nivelNutricion, setNivelNutricion] = useState('');
    const [nivelSueno, setNivelSueno] = useState('');
    ...
    const handleLogin = (username: string, password: string): boolean => {
        ...
        if (user && user.profile.password === password) {
            // Lógica para actualizar el nombre del usuario si aún tiene el valor predeterminado
            if (user.profile.name === 'Nuevo Usuario') {
                const updatedUser = {
                    ...user,
    ...
    const handleRegister = (username: string, password: string, email: string): boolean => {
        ...
        const newUser: UserData = {
            profile: { username, password, email, name: username },
            plan: {}, log: {}, monthlyGoals: {}, weeklyGoals: {},
            customWarmupExercises: {}, customMainExercises: {}, compositionLog: {}
        };
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-gray-100 font-sans">
            ...
            <main className="container mx-auto px-0 md:px-4 py-4">
                ...
                {activeTab === 'tracker' &&
                    <div>
                        {/* Aquí va tu componente de check-in diario */}
                        <section className="bg-gray-800 p-6 rounded-lg shadow-lg mb-8">
                            <h2 className="text-2xl font-bold mb-4 text-violet-400">Check-in Diario</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {/* Estado de Ánimo */}
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-300 mb-2">Estado de Ánimo</h3>
                                    <input
                                        type="text"
                                        value={estadoAnimo}
                                        onChange={(e) => setEstadoAnimo(e.target.value)}
                                        placeholder="¿Cómo te sentiste hoy?"
                                        className="w-full bg-gray-700 border-gray-600 rounded-md p-2 text-sm focus:ring-violet-500 focus:border-violet-500"
                                    />
                                </div>
                                {/* Nutrición */}
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-300 mb-2">Nutrición</h3>
                                    <div className="flex space-x-2">
                                        <button onClick={() => setNivelNutricion('Bien')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelNutricion === 'Bien' ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Bien</button>
                                        <button onClick={() => setNivelNutricion('Regular')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelNutricion === 'Regular' ? 'bg-yellow-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Regular</button>
                                        <button onClick={() => setNivelNutricion('Mal')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelNutricion === 'Mal' ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Mal</button>
                                    </div>
                                </div>
                                {/* Sueño */}
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-300 mb-2">Sueño</h3>
                                    <div className="flex space-x-2">
                                        <button onClick={() => setNivelSueno('Bien')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelSueno === 'Bien' ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Bien</button>
                                        <button onClick={() => setNivelSueno('Regular')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelSueno === 'Regular' ? 'bg-yellow-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Regular</button>
                                        <button onClick={() => setNivelSueno('Mal')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelSueno === 'Mal' ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Mal</button>
                                    </div>
                                </div>
                                {/* Energía */}
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-300 mb-2">Energía</h3>
                                    <div className="flex space-x-2">
                                        <button onClick={() => setNivelEnergia('Bien')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelEnergia === 'Bien' ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Bien</button>
                                        <button onClick={() => setNivelEnergia('Regular')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelEnergia === 'Regular' ? 'bg-yellow-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Regular</button>
                                        <button onClick={() => setNivelEnergia('Mal')} className={`flex-1 px-4 py-2 rounded-md transition-colors ${nivelEnergia === 'Mal' ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>Mal</button>
                                    </div>
                                </div>
                            </div>
                        </section>
                        ...
                        <TrackerView
                            plan={currentUserData.plan}
                            log={currentUserData.log}
                            saveDailyLog={saveDailyLog}
                            updatePlan={updatePlan}
                            estadoAnimo={estadoAnimo}
                            objetivos={{
                                mensual: currentUserData.monthlyGoals[1] || '',
                                semanal: currentUserData.weeklyGoals[1] || ''
                            }}
                            nivelNutricion={nivelNutricion}
                            nivelSueno={nivelSueno}
